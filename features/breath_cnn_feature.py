...
class BreathCNNFeature(WindowFeature,TrainableFeature):
    def __init__(self,sampling_rate,parameter_dict):
        super(BreathCNNFeature).__init__(sampling_rate,parameter_dict)
    ...
    def _process_label(self, y_window):
        ...
        spline_filtered_data = SimpleSplineFilter(avg_win=int(20/200*self._samping_rate),ds=int(20/200*self._samping_rate),s=int(45/200*self._samping_rate)).calc_feature(data_n)
        thermistor = SimpleLocalNorm(self._samping_rate,{"local_window_lenght",40}).calc_feature(spline_filtered_data)
    ...
    def train(self,train_x,train_y,val_x,val_y):
        ...
        data_x_train_list = [self._process_inputs(x) for x in train_x]
        data_y_train_list = [self._process_labels(x) for y in train_y]
    ...
    def set_params(self):
        super(BreathCNNFeature,self).set_params()
...
def main_test():
    features = SimpleLocalNorm(200,40)
    param_template = {
        "channel_count" : 10,
        "dilation" : 10,
        "feild" : 15,
